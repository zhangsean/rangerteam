...
<div id='menuActions'>
  <div class='btn-group'>
    ...
    <ul id='exportActionMenu' class='dropdown-menu'>
      <li><?php commonModel::printLink('contact', 'export', "range=all&mode={$mode}&orderBy={$orderBy}", $lang->exportAll, "class='iframe' data-width='700'");?></li>
      <li><?php commonModel::printLink('contact', 'export', "range=thispage&mode={$mode}&orderBy={$orderBy}&recTotal={$pager->recTotal}&recPerPage={$pager->recPerPage}&pageID={$pager->pageID}", $lang->exportThisPage, "class='iframe' data-width='700'");?></li>
    </ul>
  </div>
