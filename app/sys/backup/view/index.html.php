    ...
    <?php foreach($backups as $backupFile):?>
      ...
      <tr>
        ...
        <td <?php if($rowspan > 1) echo "rowspan='$rowspan'"?>>
          <?php
          if(commonModel::hasPriv('backup', 'restore')) echo html::a(inlink('restore', "file=$backupFile->name"), $lang->backup->restore, "class='restore'");
          if(commonModel::hasPriv('backup', 'delete')) echo html::a(inlink('delete', "file=$backupFile->name"), $lang->delete, "class='deleter'");
          ?>
        </td>
        <?php endif;?>
      </tr>
